...
if __name__ == '__main__':
    ...
    COMPLETION = [weighted_filling, identical_filling, uniform_filling, blank_filling]
    ...
    transition_count, kmeans, state_weightes = get_transitions(model, train_dataset, CLUSTER, True,REPLACE_RATE,DROPOUT,NUM_EPOCHS)
    ...
    for i, completion in enumerate(COMPLETION):
        for j, regularization in enumerate(REGULARIZATION):
            ...
            print(
                f'{completion.__name__} & {regularization.__name__} : {round(correct_rate * 100, 2)}%, {time.time() - current_time:.0f}s')
